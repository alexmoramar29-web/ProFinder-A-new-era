...
export default function NotificacionesClienteScreen() {
  ...
  const handleNotificationPress = async (id: string, type: string, related_id?: string) => {
    ...
    if (type === 'chat' && related_id) {
      router.push(`/(cliente)/chat/${related_id}`);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'appointment_accepted': return <Ionicons name="calendar-outline" size={24} color={colors.success.main} />;
...
const getStyles = (colors: any, isDark: boolean) => StyleSheet.create({
  ...
  tarjeta: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.background.card, padding: Spacing[4], borderRadius: Radius.md, marginBottom: Spacing[3], borderWidth: 1, borderColor: colors.border.default, ...Shadow.sm },
  tarjetaNoLeida: { backgroundColor: isDark ? colors.neutral[100] : colors.primary[50], borderColor: isDark ? colors.primary[700] : colors.primary[200] },
  iconoBox: { width: 48, height: 48, borderRadius: 24, backgroundColor: isDark ? colors.neutral[200] : colors.primary[100], justifyContent: 'center', alignItems: 'center', marginRight: Spacing[4] },
  contenidoBox: { flex: 1 },
